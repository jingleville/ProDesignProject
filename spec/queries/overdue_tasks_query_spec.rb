...
RSpec.describe OverdueTasksQuery, type: :query do
  ...
  describe ".overdue_start" do
    it "returns tasks where plan_start_at is in the past and status is approved" do
      overdue_start_task = create(:task, :approved,
        project: project,
        plan_start_at: 3.days.ago,
        plan_due_at: 5.days.from_now
      )
  ...
  describe ".overdue_deadline" do
    it "returns tasks where plan_due_at is in the past and task is not completed or cancelled" do
      overdue_task = create(:task, :in_progress,
        project: project,
        plan_due_at: 2.days.ago
      )
